...
string BKGE_NS::PlotBackground(string Interval_name, double MET, double DURATION, string FT1_FILE, string FT2_FILE, string DATACLASS, double Energy_Min_user, double Energy_Max_user, int Energy_Bins_user, bool OverwritePlots, int verbosity, double MET_FOR_THETA){
 ...
 BackgroundEstimator * Est = new BackgroundEstimator(DATACLASS ,Energy_Min_user,Energy_Max_user,Energy_Bins_user,first);
 ...
 if (CALCULATE_ROI==2) {
    if (TOOLS::ReadROI_File(hROI, TOOLS::GetS("ROI_RADIUSFILE"))) {
     ...
     for (int i=1;i<=Energy_Bins_user;i++) hROI->SetBinContent(i,TOOLS::Get("ROI_MAX_RADIUS"));
 }
 ...
 string GRB_DIR= TOOLS::GetS("OUTPUT_DIR")+"/"+GRB_NAME+"/"+Interval_name;

 char ResultsFilename[1000];
 sprintf(ResultsFilename,"%s/%s-%s_Results_%.1f_%.1f.root",GRB_DIR.c_str(),GRB_NAME.c_str(),Est->DataClass.c_str(),MET,MET+DURATION);

    //check if the estimation finished correctly first
    sprintf(name,"%s/%s_BackgroundMaps.root",GRB_DIR.c_str(),Est->DataClass.c_str());
    ...
    bool BkgOK=true;
       ...
       sprintf(name,"%s/%s_bkg_%.0f_%.0f.root",GRB_DIR.c_str(), Est->DataClass.c_str(),Energy_Min_user,Energy_Max_user);
       ftemp = fopen(name,"r");
       ...
       if (!ftemp || OverwritePlots) ProcessFile=true;
          ...
          if (ProcessFile==false) {
              if (CALCULATE_ROI==1) {
                 char PlotsFile[1000];
                 sprintf(PlotsFile,"%s/Burst_Plots.root",GRB_DIR.c_str());
	  ...
	  OverwriteResults=true;
          ...
          if (hROI->GetBinContent(1)<0) { BkgOK=false; }
          if (Est->FillBackgroundHist(GRB_DIR.c_str(), hROI,RA,DEC,3,verbosity)) {BkgOK=false;  }//both
       }
    ...
    TH1F *hROISig;
       ...
       sprintf(name,"%s/%s_sig_%.0f_%.0f.root",GRB_DIR.c_str(),Est->DataClass.c_str(),Energy_Min_user,Energy_Max_user);
       ftemp = fopen(name,"r");
       ...
       if (ProcessFile) {
	    OverwriteResults=true;
            ...
            if (FT1_FILE=='^') FT1_FILE="@"+GRB_DIR+"/fitslist.txt";
    ...
    //TEXT OUTPUT
    sprintf(name,"%s/%s_Results_%s.txt",GRB_DIR.c_str(),Est->DataClass.c_str(),GRB_NAME.c_str());
    ...
    c->cd();
     ...
     sprintf(name,"RA/DEC    : (%.3f,%.3f)deg",RA,DEC);
     ptext->AddText(name);
...
//    sprintf(name,"rm %s/*_burst_exposure.fits 2>/dev/null",GRB_DIR);    result=system(name);
    ...
    sprintf(name,"%s/%s-%s_Results_%.1f_%.1f.png",GRB_DIR.c_str(),GRB_NAME.c_str(),Est->DataClass.c_str(),MET,DURATION);
